...
export default class SearchPanel extends React.Component<SearchPanelProps, SearchPanelState> {
    ...
    private static placeRequestFields: string[] = ["name", "icon", "vicinity", "types", "photos", "website"];
    ...
    private onPlaceChanged(place: google.maps.places.PlaceResult) {
        ...
        if (Object.entries(place).length === 1) {
            ...
            this.service.textSearch(request, (results: google.maps.places.PlaceResult[], status) => {
    ...
    componentDidUpdate(previous: SearchPanelProps) {
        if (typeof (this.props.selected) === "string" && this.props.selected !== previous.selected) {
            if (!this.service) {
                this.service = new google.maps.places.PlacesService(this.props.map!);
            }

            this.service.getDetails({ fields: SearchPanel.placeRequestFields, placeId: this.props.selected }, (result, status) => {
                if (status === google.maps.places.PlacesServiceStatus.OK) {
    ...
    render() {
        ...
        if (this.props.selected && typeof (this.props.selected) === "string" && this.state.placeDetails) {
